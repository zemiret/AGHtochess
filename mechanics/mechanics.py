...
class HackyHack:
    ...
    def generate_unit(self, round: int) -> Unit:
        profession = choice(['magician', 'magicial_tank', 'tank', 'sniper'])

        def gen(min, max, multiplier: float = 1.0) -> int:
            self.count += 1
            random_value = uniform(min, max)
            self.range_value += random_value
            return int(random_value * multiplier ** (round - 1))
        ...
        def generate_magician() -> Unit:
            return Unit(attack=gen(20, 30, 1.1),
                        ...
                        type="MAGICAL",
                        price=int(uniform(.9, 1.1) * (2 * self.range_value / self.count + 1)))
